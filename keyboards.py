...
next_page_button = types.InlineKeyboardButton("▶️", callback_data="next_page")
...
SecondPageMenu.add(plan_travel_route_button)
weather_forecast_button = types.InlineKeyboardButton(text="Прогноз погоды 🌦️", callback_data="weather_forecast")
hotel_selection_button = types.InlineKeyboardButton(text="Достопримечательности 🏛️", callback_data="hotel_selection")
...
sightseeing_recommendations_button = types.InlineKeyboardButton(text="Подбор отелей 🏨", callback_data="sightseeing_recommendations")
...
SecondPageMenu.add(weather_forecast_button, hotel_selection_button, restaurant_selection_button)
ThirdPageMenu = types.InlineKeyboardMarkup(row_width=1)
ThirdPageMenu.add(ticket_booking_button, sightseeing_recommendations_button, find_travel_buddies_button)
ThirdPageMenu.add(expenses_management_button)

# Добавляем кнопки для переключения между страницами
second_page_next = types.InlineKeyboardButton("▶️", callback_data="second_page_next")
SecondPageMenu.add(second_page_next)
previous_page_button = types.InlineKeyboardButton("◀️", callback_data="previous_page")
SecondPageMenu.add(previous_page_button)


third_page_back = types.InlineKeyboardButton("◀️", callback_data="third_page_back")
ThirdPageMenu.add(third_page_back)
